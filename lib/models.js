...
class ElementMapping {
  ...
  addFieldToFieldMappingRule(sourcePath, targetPath) {
    this._rules.push(new FieldToFieldMappingRule(sourcePath, targetPath));
  }
  // withFieldToFieldMappingRule is a convenience function for chaining
  withFieldToFieldMappingRule(sourcePath, targetPath) {
    this.addFieldToFieldMappingRule(sourcePath, targetPath);
    return this;
  }

  addFieldToURLMappingRule(sourcePath, targetURL) {
    this._rules.push(new FieldToURLMappingRule(sourcePath, targetURL));
  }
  // withFieldToURLMappingRule is a convenience function for chaining
  withFieldToURLMappingRule(sourcePath, targetURL) {
    this.addFieldToURLMappingRule(sourcePath, targetURL);
    return this;
  }

  addTargetCardinalityMappingRule(targetPath, cardinality) {
    this._rules.push(new TargetCardinalityMappingRule(targetPath, cardinality));
  }
  // withTargetCardinalityMappingRule is a convenience function for chaining
  withTargetCardinalityMappingRule(targetPath, cardinality) {
    this.addTargetCardinalityMappingRule(targetPath, cardinality);
    return this;
  }
...
class FieldToFieldMappingRule {
  constructor(sourcePath = [], targetPath = []) {
    this._sourcePath = sourcePath; // array of identifiers
    this._targetPath = targetPath; // array of strings
  }
  ...
  clone() {
    ...
    const clonedTP = this._targetPath.map(p => p);
    return new FieldToFieldMappingRule(clonedSP, clonedTP);
  }
}

class FieldToURLMappingRule {
  constructor(sourcePath = [], targetURL) {
    this._sourcePath = sourcePath; // array of identifiers
    this._targetURL = targetURL;   // URL string
  }
  ...
  clone() {
    const clonedSP = this._sourcePath.map(p => p.clone());
    return new FieldToURLMappingRule(clonedSP, this._targetURL);
  }
}

class TargetCardinalityMappingRule {
  constructor(targetPath = [], cardinality) {
    this._targetPath = targetPath;   // array of strings
  ...
  clone() {
    const clonedTP = this._targetPath.map(p => p);
    return new TargetCardinalityMappingRule(clonedTP, this._cardinality.clone());
  }
}
...
class MappingRulesFilter {
  ...
  get fieldToField() {
    return new MappingRulesFilter(this._rules.filter(c => c instanceof FieldToFieldMappingRule));
  }

  get fieldToURL() {
    return new MappingRulesFilter(this._rules.filter(c => c instanceof FieldToURLMappingRule));
  }

  get targetCardinality() {
    return new MappingRulesFilter(this._rules.filter(c => c instanceof TargetCardinalityMappingRule));
  }
  ...
  withTargetPath(path = []) {
    const matches = this._rules.filter(r => {
      if (typeof r.targetPath === 'undefined') {
        return false;
      }
      if (path.length != r.targetPath.length) {
        return false;
      }
      for (let i=0; i < path.length; i++) {
        if (path[i] != r.targetPath[i]) {
          return false;
        }
      }
      return true;

...
//console.log("current namespace = "+JSON.stringify(currentNamespace));
    ...
    const formattedDataElementFileHash = namespaceFormatter.formatDataElementFile(substituteHash);
    for (let k in formattedDataElementFileHash) {
      this.exportDataElementFileToNamespace(formattedDataElementFileHash[k], k.split(".")[0], path);
    }

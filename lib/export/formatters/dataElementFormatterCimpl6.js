...
class DataElementFormatterCimpl6 {
  ...
  formatDeclaration(de) {
    ...
    const formattedTitle = `${rightPad(`${formattedModifier}:`, this.maxRightPad)}${de.identifier.name}`;
    return formattedTitle;
  }
      ...
      if (f.inheritance != 'inherited') {
        ...
        if (Object.keys(constraintsByPaths).length > 0) {
          for (const path in constraintsByPaths) {
            ...
            let num_new_line = inserted_value.split(/\r\n|\r|\n/).length
            if (inserted_value.includes('0..0')) {
              formattedFields.push('Forbid: ' + inserted_value.replace('0..0', ''));
            }
            else {
              if (f.inheritance == 'overridden') {
                if (num_new_line == 1) {
                  formattedFields.push('Constraint: ' + inserted_value);
                }
                else {
                  formattedFields.push('Populate: ' + inserted_value)
                }
              }
              else {
                formattedFields.push('Property: ' + inserted_value);
              }
            }
          }
        } else {
  ...
  formatValue(value, de, formatAsField, path) {
    ...
    if (!formatAsField) {
      return `${rightPad('Value:',this.maxRightPad)}${formattedValue}`.replace(/[0-9]+..[0-9]+/, '');
    }
  ...
  formatValueWithConstraints(formattedByPath, value, de, formatAsField) {
    ...
    formattedWithConstraint = formattedWithConstraint.replace('Value:', 'Value accept only');
    formattedWithConstraint = this.replaceValue(formattedWithConstraint);
  ...
  formatValueByConstraintType(formattedValue, constraint, formatAsField, value) {
    ...
    case 'CardConstraint': {
      if (formatAsField) {
        return this.formatValueWithoutCardConstraint(formattedValue, constraint);
      } else {
        return formattedValue;
      }
    }
  ...
  formatValueWithTypeConstraint(formattedValue, constraint) {
    ...
    if (constraint.onValue) {
      return `${formattedValue} .Value accept only ${constraint.isA.name}`;
    } else {
      return `${formattedValue} substitute ${constraint.isA.name}`;
    }
  }
  ...
  formatValueWithoutCardConstraint(formattedValue, constraint, formatAsField) {
  ...
  formatValueWithCodeConstraint(formattedValue, constraint) {
    ...
    if (constraint.path.length >= 2 && constraint.path[0].name == 'Units' && constraint.path[1].name == 'Coding') {
      return `${formattedValue}.Units.Value = ${formattedCode}`;
    }
  ...
  formatValueWithIncludesTypeConstraint(formattedValue, constraint) {
    let formattedConstraint = `${rightPad('\t',this.maxRightPad)} ${constraint.card.toString()} ${constraint.isA.name}`;

...
class DataElementFormatterCimpl6 {
  ...
  formatFields(de) {
        ...
        if (Object.keys(constraintsByPaths).length > 0) {
          for (const path in constraintsByPaths) {
            ...
            if (f.inheritance == 'overridden') {
              constrainedFields.push(inserted_value);
            }
            else {
              newFields.push(inserted_value);
            }
          }
        } else {
    ...
    let a = 0;
    for(a = 0; a < newFields.length; a++) {
      formattedFields.push(newFields[a]);
    }
    if (constrainedFields.length > 0) {
      formattedFields.push('Constrain:');
      for(a = 0; a < constrainedFields.length; a++) {
        formattedFields.push(constrainedFields[a]);
      }
    }
  ...
  formatValue(value, de, formatAsField, path) {
    ...
    if ('card' in value) {
      if (value.effectiveCard.max == 0) {
        if (!formatAsField) {
          formatAsField = true;
          return `${rightPad('Forbid:', this.maxRightPad)}Value`;
        } else {
          return `${rightPad('Forbid:', this.maxRightPad)}${formattedValue}`;
        }
      }

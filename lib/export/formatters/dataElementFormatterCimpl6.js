...
class DataElementFormatterCimpl6 {
  ...
  extractValueName(formattedValue) {
    let valueName = '';
  ...
  formatValue(value, de, formatAsField, path, substituteHash) {
    ...
    if (path && path !== 'root' && value.inheritance && !path.includes('[')) {
      ...
      if(formattedValue.includes('Units.')) {
        console.log(`inside the modificiation we have ${formattedValue}`);
      }
      let modifiedFormattedValue = splitFormattedValue[0].trim();
      let subName = "";
      let subNs = "";
      ...
      for (let i = 1; i < splitFormattedValue.length; i++) {
        ...
        if(formattedValue.includes('Units.')) {
          console.log(`after the first check inside the modificiation we have ${formattedValue}`);
          console.log(modifiedFormattedValue);
        }
        if (!added) {
          if (newDe.value) {
            if (newDe.value.options) {
    ...
    if ('constraints' in value && value.constraints.length > 0) {
      ...
      if(formattedValue.includes('Units.')) {
        console.log(`right now inside another function ${formattedValue}`);
        console.log(path);
      }
    ...
    if (!formatAsField) {
      if (formattedValue.includes('with units')) {
        ...
        let constraintPath = splitFormattedValue[0].split(".")
        let i = 0;
        let valueName = '';
        while(i < splitFormattedValue[0].length && splitFormattedValue[0][i] != '.' && splitFormattedValue[0][i] != '[') {
          valueName += splitFormattedValue[0][i];
  ...
  formatValueWithConstraints(formattedByPath, value, de, formatAsField, substituteHash) {
    ...
    for (const con of value.constraints) {
      if (con.fqn == de.fqn || con.lastModifiedBy.fqn == de.identifier.fqn) {
        if(formattedByPath.includes('Units.') && con.constructor.name == 'CardConstraint') {
          console.log(`over here we have ${formattedByPath}`);
          console.log(con.path);
          //console.log(hhh);
        }
        ...
        if (!formattedByPathCopy.includes('[')) {
          if(formattedByPathCopy.includes('Units.') && con.constructor.name == 'CardConstraint') {
            console.log(`over here before another update we have ${formattedByPathCopy}`);
            console.log(con.path);
          }
        ...
        if (con.constructor.name === 'IncludesTypeConstraint') {
          ...
          if(formattedByPathCopy.trim() == 'Units' && con.constructor.name == 'CardConstraint') {
            //console.log("units over here!!");
            //console.log(con);
          }

...
class DataElementFormatterCimpl6 {
  ...
  formatDeclaration(de) {
    ...
    if (de.isEntry) {
      ...
      return `${rightPad(`${formattedModifier}Element:`, this.maxPlacement - 6 - formattedModifier.length)}${de.identifier.name.trim()}`;
    }
  ...
  formatDescription(de) {
    return `${rightPad('Description:', this.maxPlacement - 9)}"${de.description.trim()}"`;
  }
              ...
              if (fClone.constraints.length > 0) {
                ...
                for (i = 0; i < fClone.constraints.length; i++) {
                  if (fClone.constraints[i].valueSet !== undefined) {
                    ...
                    constrainedFields.push(`${rightPad('', this.maxPlacement)}${this.formatValueByConstraintType(fClone.identifier.name, fClone.constraints[i], false, inserted_value, substituteHash)}`);
                  }
                }
              }
              else {
            ...
            if (f.constraints.length > 0) {
              ...
              for (i = 0; i < f.constraints.length; i++) {
                if (f.constraints[i].valueSet !== undefined) {
                  ...
                  constrainedFields.push(`${rightPad('', this.maxPlacement)}${this.formatValueByConstraintType(f.identifier.name, f.constraints[i], false, inserted_value, substituteHash)}`);
                }
              }
            }
            else {
    ...
    for (const con of value.constraints) {
      if (con.lastModifiedBy.fqn == de.identifier.fqn) {
        ...
        if (con.constructor.name === 'IncludesTypeConstraint') {
          ...
          formattedWithConstraint = this.formatValueByConstraintType(formattedByPath, con, formatAsField, value, substituteHash);
        }
  ...
  formatValueByConstraintType(formattedValue, constraint, formatAsField, value, substituteHash = {}) {
    formattedValue = this.replaceValue(formattedValue);
    ...
    case 'TypeConstraint': {
      ...
      return this.formatValueWithTypeConstraint(formattedValue, constraint, substituteHash);
    }
  ...
  formatValueWithTypeConstraint(formattedValue, constraint, substituteHash) {
    ...
    if (constraint.onValue) {
      ...
      if (!substituteHash.hasOwnProperty(constraint.isA.namespace)) {
        substituteHash[constraint.isA.namespace] = {};
      }
      substituteHash[constraint.isA.namespace][formattedValue] = constraint.isA.name;
      //console.log(substituteHash);
      this.addPair(formattedValue, constraint.isA.name);

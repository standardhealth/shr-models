  ...
  */
  getParentsSubstitution(de, substituteHash, term) {
    ...
    */
    while(newParents.length > 0) {
      ...
      //console.log(currentParent.identifier.name);
      ...
      if(deNamespace) {
        ...
        if (deName) {
          if (deName.hasOwnProperty(term)) {
            console.log("match made over here");
            return deName[term];
          }
        }
      }
      if(currentParent.basedOn) {
        ...
        for(let i = 0; i < basedOnElements.length; i++) {
          //console.log(this.specs.dataElements.find(basedOnElements[i].namespace, basedOnElements[i].name));
          newParents.push(this.specs.dataElements.find(basedOnElements[i].namespace, basedOnElements[i].name));
        }
      }
      depth += 1
    }
    //console.log(`in the end the depth we went is ${depth}`);
    return term
  }
                    ...
                    for(let i = 0; i < split_value.length; i++) {
                      if(split_value[i].length > 0) {
                        if(deName.hasOwnProperty(split_value[i])) {
                          split_value[i] = deName[split_value[i]];
                        }
                        else {
                          split_value[i] = this.getParentsSubstitution(de, substituteHash, split_value[i]);
                        }
                      }
                    }
                  }
  ...
  formatValueWithTypeConstraint(formattedValue, constraint, substituteHash, de) {
    ...
    if (constraint.onValue) {
      ...
      substituteHash[de.identifier.namespace][de.identifier.name][formattedValue.trim()] = constraint.isA.name;

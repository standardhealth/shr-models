...
class DataElementFormatterCimpl6 {
    ...
    for (const con of value.constraints) {
      if (con.lastModifiedBy.fqn == de.identifier.fqn) {
        ...
        if (formattedWithConstraint.includes('substitute')) {
          console.log(formattedWithConstraint);
          console.log(formattedWithConstraint.indexOf('StatementTopic'));
        }
  ...
  formatValueWithTypeConstraint(formattedValue, constraint) {
    ...
    if (constraint.onValue) {
      ...
      let t = `${rightPad('', this.maxPlacement)}${formattedValue.trim()} substitute ${constraint.isA.name}`;
      console.log(t);

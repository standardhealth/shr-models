...
class DataElementFormatterCimpl6 {
  ...
  shFromCodeSystem(cs) {
    ...
    if (shorthands[cs]) {
      ...
      console.log(`error over here found ${cs}`);
      return cs;
    }
  }
...
//     return vs;
//    }
// Preserving the elements that seems to be functional, but the purpose is unclear
    const name = vs.split('/').pop().replace('#', '');
    return name;
// End MK: 3/22/2019
  }
        ...
        if (Object.keys(constraintsByPaths).length > 0) {
          ...
          if (f.inheritance == 'overridden') {
            ...
            if (f.constraints.length > 0) {
              if (!f.identifier.name.includes('concept')) {

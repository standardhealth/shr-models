...
class DataElementFormatterCimpl6 {
  ...
  shFromCodeSystem(cs) {
    ...
    if (shorthands[cs]) {
      ...
      for (let k in shorthands) {
        if (shorthands[k].includes('UNK')) {
          unknown_terms.push(parseInt(shorthands[k].slice(3)));
        }
      }
      if (unknown_terms.length > 0) {
        shorthands[cs] = "UNK" + (Math.max(unknown_terms) + 1)
      }
      else {
        shorthands[cs] = "UNK1";
      }
  ...
  getIdFromFqnAndTrackUsed(fqn, de) {
    const id = idFromFQN(fqn);
    if (id.namespace != 'primitive' && de.identifier.namespace != id.namespace) {
...
// MK: 3/22/2019: The following code bombs because the pattern match is null and can't be referenced at index [1]

     const matchArray = vs.match(/standardhealthrecord\.org\/(.*)\/vs\/#[A-Za-z0-9]*/);
    if(matchArray && matchArray.length > 1 && matchArray[1]) {
  ...
  formatBasedOns(de) {
    ...
    for (const basedOn of de.basedOn) {
      ...
      const formattedBasedOn = this.getIdFromFqnAndTrackUsed(basedOn.fqn, de).name;
...
//      if (constraint.lastModifiedBy.fqn != de.identifier.fqn) {
      ...
      if (constraint.path.length > 0) {
        for (let i = 0; i < constraint.path.length; i++) {
          if (constraint.path[i].fqn) this.getIdFromFqnAndTrackUsed(constraint.path[i].fqn, de);
  ...
  */
        ...
        if (Object.keys(constraintsByPaths).length > 0) {
          for (const path in constraintsByPaths) {
            ...
            if (f.inheritance == 'overridden') {
              ...
              if (fClone.constraints.length > 0) {
                if (!fClone.identifier.name.includes('concept')) {
                ...
                for (i = 0; i < fClone.constraints.length; i++) {
                  let propertyConstraint = fClone.identifier.name;
                  if (fClone.constraints[i].path && fClone.constraints[i].path.length > 0) {
                    let propertyArr = propertyConstraint.split(" ");
                    let firstPart = propertyArr[0];
                    /* Check if path is already present which occurs in the case of
                    exporting CIMPL 5 files to CIMPL 6.
                    */
                    if (!firstPart.includes(".")) {
                      for (let j = 0; j < fClone.constraints[i].path.length - 1; j++) {
                        firstPart += `.${fClone.constraints[i].path[j].name}`;
                      }
                      if (fClone.constraints[i].path[fClone.constraints[i].path.length - 1].name != 'concept') {
                        firstPart += `.${fClone.constraints[i].path[fClone.constraints[i].path.length - 1]}`;
                      }
                      firstPart = this.updatePath(firstPart, de);
                      if (formattedValueArr.length > 1) {
                        firstPart += " " + formattedValueArr.slice(1).join(' ');
                      }
                      propertyConstraint = firstPart;
                    }
                  }
                ...
                if (!inserted_value.includes('concept')) {
          ...
          if (f.inheritance == 'overridden') {
            ...
            if (f.constraints.length > 0) {
              if (!f.identifier.name.includes('concept') && f.card) {
              ...
              for (i = 0; i < f.constraints.length; i++) {
                let propertyConstraint = f.identifier.name;
                if (f.constraints[i].path && f.constraints[i].path.length > 0) {
                  let propertyArr = propertyConstraint.split(" ");
                  let firstPart = propertyArr[0];
                  /* Check if path is already present which occurs in the case of
                  exporting CIMPL 5 files to CIMPL 6.
                  */
                  if (!firstPart.includes(".")) {
                    for (let j = 0; j < f.constraints[i].path.length - 1; j++) {
                      firstPart += `.${f.constraints[i].path[j].name}`;
                    }
                    if (f.constraints[i].path[f.constraints[i].path.length - 1].name != 'concept') {
                      firstPart += `.${f.constraints[i].path[f.constraints[i].path.length - 1].name}`;
                    }
                    firstPart = this.updatePath(firstPart, de);
                    if (propertyArr.length > 1) {
                      firstPart += " " + propertyArr.slice(1).join(' ');
                    }
                    propertyConstraint = firstPart;
                  }
                }
                if (f.constraints[i].valueSet !== undefined) {
                  constrainedFields.push(`${rightPad('', this.maxPlacement)}${propertyConstraint} from ${f.constraints[i].valueSet} (${f.constraints[i].bindingStrength.toLowerCase()})`);
                }
                else {
                  constrainedFields.push(`${rightPad('', this.maxPlacement)}${this.formatValueByConstraintType(propertyConstraint, f.constraints[i], false, inserted_value, substituteHash, de)}`);
                }
              }
            }
            else {
  ...
  formatValue(value, de, formatAsField, path, substituteHash) {
    ...
    if (path && path !== 'root' && value.inheritance) {
      formattedValue = this.updatePath(`${formattedValue}.${path}`, de);
    }
    ...
    if (!formatAsField) {
      ...
      let finalValueStringsLis = [];
      //finalValueStringsLis.push(`${rightPad('Value:',this.maxPlacement - 6)}${originalValue}`);
      let formattedValueParts = formattedValue.split('\n');
      if ('constraints' in value && value.constraints.length > 0) {
        for(let a = 0; a < formattedValueParts.length; a++) {
          let trimmedPart = formattedValueParts[a].trim().split(" ");
          if (a == 0) {
            if (value.constraints[a].constructor.name != 'ValueSetConstraint') {
              finalValueStringsLis.push(`${rightPad('Value:',this.maxPlacement - 6)}${originalValue}`);
            }
            else {
              finalValueStringsLis.push(`${rightPad('Value:',this.maxPlacement - 6)}${formattedValueParts[a].trim()}`);
              continue;
            }
          }
          let valuePart = `Value[${originalValue.trim()}]`;
          let firstIndex = trimmedPart[0].indexOf(originalValue);
          firstIndex += originalValue.length;
          if (firstIndex < trimmedPart[0].length - 1) {
            valuePart += trimmedPart[0].substr(firstIndex);
          }
          valuePart += " " + trimmedPart.slice(1).join(" ");
          finalValueStringsLis.push(`${rightPad('',this.maxPlacement)}${valuePart}`);
        }
      }
      else {
        finalValueStringsLis.push(`${rightPad('Value:',this.maxPlacement - 6)}${originalValue}`);
      }
      return finalValueStringsLis.join('\n')
    }
  ...
  formatValueByType(value, de, substituteHash) {
    if (value.identifier && value.identifier.fqn) {
      this.getIdFromFqnAndTrackUsed(value.identifier.fqn, de);
    }
    ...
    for (const con of value.constraints) {
      if (con.fqn == de.fqn || con.lastModifiedBy.fqn == de.identifier.fqn) {
        ...
        if (con.constructor.name === 'IncludesTypeConstraint') {
          ...
          //console.log(`formattedByPath is ${formattedByPath}`);
          if (con.path && con.path.length > 0) {
            formattedByPath = formattedByPath.replace(/[0-9]+..[0-9]+/, '')
            let formattedValueArr = formattedByPath.split(" ");
            let firstPart = formattedValueArr[0];
            /* Check if path is already present which occurs in the case of
            exporting CIMPL 5 files to CIMPL 6.
            */
            if (!firstPart.includes(".")) {
              for (let i = 0; i < con.path.length - 1; i++) {
                firstPart += `.${con.path[i].name}`;
              }
              if (con.path[con.path.length - 1].name != 'concept') {
                firstPart = `${firstPart}.${con.path[con.path.length - 1].name}`;
              }
              firstPart = this.updatePath(firstPart, de);
              if (formattedValueArr.length > 1) {
                firstPart += " " + formattedValueArr.slice(1).join(' ');
              }
              formattedByPath = firstPart;
            }
          }
    ...
    */
    case 'ValueSetConstraint': {
      if (!formatAsField && constraint.path.length >= 2 && constraint.path[0].name == 'Units' && constraint.path[1].name == 'Coding') {
        return [
          formattedValue,
          `${rightPad('', this.maxRightPad)}${this.formatValueWithValueSetConstraint(`${formattedValue.replace(/.*\s([^\s]*)/,'$1')}.Units`, constraint, de)}`,
        ].join('\r\n');
      }

      return this.formatValueWithValueSetConstraint(formattedValue, constraint, de);
    }
    case 'CardConstraint': {
  ...
  formatValueWithTypeConstraint(formattedValue, constraint, substituteHash, de) {
    if (constraint.isA.fqn) {
      this.getIdFromFqnAndTrackUsed(constraint.isA.fqn, de);
    }
  ...
  formatValueWithValueSetConstraint(formattedValue, constraint, de) {
    let vs = constraint.valueSet;

    if (vs.identifier && vs.identifier.fqn) {
      this.getIdFromFqnAndTrackUsed(vs.identifier.fqn, de);
    }

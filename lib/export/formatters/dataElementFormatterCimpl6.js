              ...
              if (fClone.constraints.length > 0) {
                ...
                for (i = 0; i < fClone.constraints.length; i++) {
                  ...
                  if (fClone.constraints[i].valueSet !== undefined) {
                    formattedFields.push(fClone.identifier.name + " from " + fClone.constraints[i].valueSet + " (" + fClone.constraints[i].bindingStrength.toLowerCase() + ")");
                  }
                }
              }
              else {
  ...
  formatValueWithConstraints(formattedByPath, value, de, formatAsField) {
    ...
    for (const con of value.constraints) {
      if (con.lastModifiedBy.fqn == de.identifier.fqn) {
        let formattedWithConstraintTerm = this.formatValueByConstraintType(formattedWithConstraint, con, formatAsField, value);
        formattedWithConstraintTerms.push(this.replaceValue(formattedWithConstraintTerm));
      }
    }

...
class DataElementFormatterCimpl6 {
  ...
  set substituteHash(pair) { this._substituteHash[pair[0]] = pair[1]}
  ...
  formatDeclaration(de) {
    ...
    if (de.isEntry) {
      ...
      const formattedTitle = `${rightPad(`${formattedModifier}Element:`, this.maxPlacement - 6 - formattedModifier.length)}${de.identifier.name.trim()}`;
      return formattedTitle;
    }
    const formattedTitle = `${rightPad(`${formattedModifier}:`, this.maxPlacement - formattedModifier.length)}${de.identifier.name.trim()}`;
    return formattedTitle;
  }
  ...
  formatBasedOns(de) {
    ...
    return `${rightPad('Parent:', this.maxPlacement - 6)}${formattedBasedOns.join(', ')}`;
  }
  ...
  formatConcepts(de) {
    ...
    return `${rightPad('Concept:', this.maxPlacement - 6)}${formattedConcepts.join(', ')}`;
  }

  formatDescription(de) {
    const formattedDesc = `${rightPad('Description:', this.maxPlacement - 9)}"${de.description.trim()}"`;
        ...
        if (Object.keys(constraintsByPaths).length > 0) {
          for (const path in constraintsByPaths) {
            ...
            if (f.inheritance == 'overridden') {
              for (var key in this.substituteHash) {
                if (inserted_value.includes(key) && !inserted_value.includes('substitute')) {
                  inserted_value = inserted_value.replace(key, this.substituteHash[key])
                }
              }
              let constrainedTerm = `${rightPad('', this.maxPlacement)}${inserted_value.trim()}`;
              constrainedFields.push(`${rightPad('', this.maxPlacement)}${inserted_value.trim()}`);
            }
            else {
              if (fClone.constraints.length > 0) {
                formattedFields.push(`${rightPad('Property:', this.maxPlacement - 7)}${fClone.identifier.name} ${fClone.card.toString()}`);
                let i = 0;
                ...
                formattedFields.push(`${rightPad('Property:', this.maxPlacement - 7)}${inserted_value.trim()} ${fClone.card.toString()}`);
              }
            }
          }
        } else {
          ...
          if (f.inheritance == 'overridden') {
            for (var key in this.substituteHash) {
              if (inserted_value.includes(key)) {
                inserted_value = inserted_value.replace(key, this.substituteHash[key])
              }
            }
            constrainedFields.push(`${rightPad('', this.maxPlacement)}${inserted_value.trim()}`);
            ...
            if (f.constraints.length > 0) {
              formattedFields.push(`${rightPad('Property:', this.maxPlacement - 7)}${f.identifier.name}`);
              let i = 0;
              ...
              formattedFields.push(`${rightPad('Property:', this.maxPlacement - 7)}${inserted_value.trim()}`);
            }
          }
        }
      }
  ...
  formatValue(value, de, formatAsField, path) {
    ...
    if (path && path !== 'root' && value.inheritance) {
      ...
      for (i = 0; i < splitFormattedValue.length - 1; i++) {
        let added = false;
        subName = splitFormattedValue[i + 1].trim()
        for(j = 0; j < newDe.fields.length; j++) {
          if (splitFormattedValue[i + 1].trim() === newDe.fields[j].identifier.name) {
            modifiedFormattedValue += `.${splitFormattedValue[i + 1]}`;
            subNs = newDe.fields[j].identifier.namespace;
            added = true;
            break;
          }
          else if (splitFormattedValue[i + 1].trim() === newDe.fields[j].effectiveIdentifier.name) {
            modifiedFormattedValue += `.${splitFormattedValue[i + 1]}`;
        ...
        if (!added) {
          if (newDe.value) {
            if (newDe.value.options) {
              let a = 0;
              for(a = 0; a < newDe.value.options.length; a++) {
                if (splitFormattedValue[i +1].trim() === newDe.value.options[a].identifier.name) {
                  modifiedFormattedValue += `[${splitFormattedValue[i + 1]}]`;
                  subNs = newDe.value.options[a].identifier.namespace;
                }
                else if (splitFormattedValue[i +1].trim() === newDe.value.options[a].effectiveIdentifier.name) {
                  modifiedFormattedValue += `[${splitFormattedValue[i + 1]}]`;
              ...
              if (splitFormattedValue[i + 1].trim() === newDe.value.identifier.name) {
                modifiedFormattedValue += `[${splitFormattedValue[i + 1]}]`;
                subNs = newDe.value.identifier.namespace;
              }
              else if (splitFormattedValue[i + 1].trim() === newDe.value.effectiveIdentifier.name) {
                modifiedFormattedValue += `[${splitFormattedValue[i + 1]}]`;
      ...
      if (!formatAsField && value.inheritance === 'overridden') {
        if (de.identifier.name === value.constraints[0].lastModifiedBy.name) {
          ...
          if (formattedValueParts.length > 1) {
            firstPart += `${rightPad('',this.maxPlacement)}Value ${formattedValueParts.slice(1).join(" ")}`;
          }
    ...
    if (!formatAsField) {
      if (formattedValue.includes('with units')) {
        let splitFormattedValue = formattedValue.split(' ');
        let formattedStr = `${rightPad('Value:', this.maxPlacement - 5)}${splitFormattedValue[0]}`;
      ...
      return `${rightPad('Value:',this.maxPlacement - 5)}${formattedValue}`.replace(/[0-9]+..[0-9]+/, '');
    }
  ...
  formatValueWithTypeConstraint(formattedValue, constraint) {
    ...
    if (constraint.onValue) {
      return `${rightPad('',this.maxPlacement)}${formattedValue.trim()} only ${constraint.isA.name}`;
    }
    else {
      this.substituteHash = [formattedValue, constraint.isA.name];
      return `${rightPad('', this.maxPlacement)}${formattedValue.trim()} substitute ${constraint.isA.name}`;
    }
  }
  ...
  formatValueWithBooleanConstraint(formattedValue, constraint) {
    return `${rightPad('',this.maxPlacement)}${formattedValue} = ${constraint.value}`;
  }
  ...
  formatValueWithCodeConstraint(formattedValue, constraint) {
    ...
    let j = `${formattedValue} = ${formattedCode}`;
  ...
  formatValueWithIncludesTypeConstraint(formattedValue, constraint) {
    let formattedConstraint = `${rightPad('',this.maxPlacement)}includes ${constraint.isA.name} ${constraint.card.toString()}`;
    if (formattedValue) {

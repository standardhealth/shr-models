...
class DataElementFormatterCimpl6 {
          ...
          for (const path in constraintsByPaths) {
            ...
            if (f.inheritance == 'overridden') {
              ...
              if (fClone.constraints.length > 0) {
                formattedFields.push('Property:          ' + fClone.identifier.name);
                let i = 0;
                for (i = 0; i < fClone.constraints.length; i++) {
                  formattedFields.push(fClone.identifier.name + " from " + fClone.constraints[i].valueSet + " (" + fClone.constraints[i].bindingStrength + ")");
                }
              }
              else {
          ...
          if (f.inheritance == 'overridden') {
            ...
            if (f.constraints.length > 0) {
              ...
              for (i = 0; i < f.constraints.length; i++) {
                formattedFields.push(f.identifier.name + " from " + f.constraints[i].valueSet + " (" + f.constraints[i].bindingStrength + ")");
              }
            }
            else {
              formattedFields.push('Property:          ' + inserted_value.trim());
  ...
  formatValue(value, de, formatAsField, path) {
    ...
    if ('constraints' in value && value.constraints.length > 0) {
      ...
      if (!formatAsField) {
        ...
        let firstPart = `${rightPad('Value only',this.maxRightPad)}${formattedValueParts[0]}\n`
  ...
  formatValueWithTypeConstraint(formattedValue, constraint) {
    ...
    if (constraint.onValue) {
      return `${formattedValue} only ${constraint.isA.name}`;
    } else {
  ...
  formatValueWithValueSetConstraint(formattedValue, constraint) {
    ...
    if (this.specs._valueSets.findByURL(vs) !== undefined) {
      vs = constraint._lastModifiedBy._name;
    }

  ...
  getParentsSubstitution(de, substituteHash, term) {
    ...
    */
    while(newParents.length > 0) {
      /*if (term == 'QuestionnaireItem') {
        let deNamespace = substituteHash[currentParent.identifier.namespace];
        if (deNamespace) {
          console.log(`currentParent identifier is ${currentParent.identifier.name} and ns is ${currentParent.identifier.namespace}`);
          let deName = deNamespace[currentParent.identifier.name];
          if (deName) {
            console.log(`term defined here is ${term}`);
            console.log(`deName found over here is`);
            console.log(deName);
            if (deName.hasOwnProperty(term)) {
              console.log("the key was found here");
            }
          }
        }
      }*/
      ...
      if(currentParent.basedOn) {
        ...
        for(let i = 0; i < basedOnElements.length; i++) {
          ...
          if (term == 'Identifier') {
            console.log(`for the current parent ${currentParent.identifier.name} the parent is ${basedOnElements[i].name}`);
          }
        }
      }
      depth += 1
                ...
                for(let i = 0; i < split_value.length; i++) {
                  if(split_value[i].length > 0) {
                    ...
                    if(split_value[i][split_value[i].length - 1] === ']') {
                      ...
                      if(deName) {
                        if (split_value[i].includes('Identifier')) {
                          console.log(`within first block namespace found is ${de.identifier.namespace}`);
                        }
                        if(deName.hasOwnProperty(split_value[i])) {
                          split_value[i] = `${firstPart}${deName[split_value[i]]}`;
                        }
                        else if (deName.hasOwnProperty(firstPart)) {
                          split_value[i] = `${deName[firstPart]}${split_value[i].substring(last_index)}`;
                        }
                        else {
              ...
              for(let i = 0; i < split_value.length; i++) {
                if(split_value[i].length > 0) {
                  ...
                  if(split_value[i][split_value[i].length - 1] === ']') {
                    ...
                    if (split_value[i].includes('Identifier')) {
                      console.log(`in the second block ns is ${de.identifier.namespace}`);
                    }
                    if(deName) {
                      if(deName.hasOwnProperty(split_value[i])) {
                        split_value[i] = `${firstPart}${deName[split_value[i]]}`;

  ...
  formatValue(value, de, formatAsField, path, substituteHash) {
      ...
      if (!formatAsField && value.inheritance === 'overridden') {
        if (de.identifier.name === value.constraints[0].lastModifiedBy.name) {
          ...
          if (formattedValueParts.length > 1) {
            firstPart += `${rightPad('',this.maxPlacement)}Value[${formattedValueParts[0]}] ${formattedValueParts.slice(1).join(" ").trim()}`;
          }
    ...
    if (!formatAsField) {
      if (formattedValue.includes('with units')) {
        ...
        let formattedStr = `${rightPad('Value:', this.maxPlacement - 6)}${constraintPath[0]}`;
        formattedStr += `\n${rightPad('',this.maxPlacement)}Value[${constraintPath[0]}].${constraintPath.slice(1).join(".")} = ${splitFormattedValue[splitFormattedValue.length - 1]}`.replace(/[0-9]+..[0-9]+/, '');
        return formattedStr;
      }
  ...
  formatValueWithConstraints(formattedByPath, value, de, formatAsField, substituteHash) {
    ...
    for (const con of value.constraints) {
      if (con.fqn == de.fqn || con.lastModifiedBy.fqn == de.identifier.fqn) {
        ...
        if (!(formattedByPath.includes('.') || formattedByPath.includes('['))) {

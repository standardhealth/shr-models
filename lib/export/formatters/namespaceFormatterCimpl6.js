...
class FileHeaderFormatter {
  ...
  formatFileDeclaration(fileType, ns, target, maxRightPad = 4) {
    const formattedDeclaration = [
      `${rightPad('Grammar:', DEFAULT_MAX_PLACE - 7)}${fileType} 6.0`,
      `${rightPad('Namespace:', DEFAULT_MAX_PLACE - 9)}${ns.namespace.trim()}`,
    ];

    if (fileType === 'DataElement') {
      if (ns.description) {
        formattedDeclaration.push(`${rightPad('Description:', DEFAULT_MAX_PLACE - 10)}"${ns.description.trim()}"`);
      }
  ...
  formatUses(uses, maxRightPad) {
    return `${rightPad('Uses:', DEFAULT_MAX_PLACE - 5)}${uses.join(', ')}`;
  }

  formatCodeSystem(cs, maxRightPad) {
    return `${rightPad('CodeSystem:', DEFAULT_MAX_PLACE - 10)}${shorthandFromCodesystem(cs)} = ${cs.trim()}`;
  }

  formatPath(path, maxRightPad) {
    return `${rightPad('Path:', DEFAULT_MAX_PLACE - 5)}${path.shorthand} = ${path.url.trim()}`;
  }
}

  ...
  formatDataElementFile(substituteHash) {
    ...
    this.specs.dataElements.files.forEach(f => {
      ...
      this.specs.dataElements.byFile(f).forEach(de => {
        ...
        console.log(f);
        console.log(this.dataElementFormatter.uses);
        console.log(this.dataElementFormatter.codesystems);
      });
      const header = this.formatHeader('DataElement', file_ns);

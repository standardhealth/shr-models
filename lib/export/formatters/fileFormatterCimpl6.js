...
class FileFormatterCimpl6 {
  ...
  formatHeader(fileType, file_ns, file_uses, file_cs) {
    ...
    case 'ValueSet': {
      return this.headerFormatter.formatValueSetFileHeader(file_ns, file_cs);
    }
    case 'Map': {
      return this.headerFormatter.formatMappingFileHeader(file_ns, this.mappingFormatter.target);
    }
    default:
      return;
    }
  ...
  formatDataElementFile(substituteHash) {
    ...
    this.specs.dataElements.files.forEach(f => {
      formattedDataElementsHash[f] = [];
      const fileDataElements = [];
      let file_ns = ''
      let num_de = this.specs.dataElements.byFile(f).length;
      let c = 0;
      ...
      formattedDataElementsHash[f].push([header, ...fileDataElements].join('\r\n\r\n'));
      this.dataElementFormatter.reset();
    });
  ...
  formatValueSetFile() {
    ...
    //const fileHeader = this.formatHeader('ValueSet', vs.identifier.namespace);

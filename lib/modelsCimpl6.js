  ...
  find(namespace, name) {
    ...
    if (namespace === '') {
      ...
      if (id.isEntryKeyWord) {
        ...
        [namespace, name] = ['shr.core', 'conceptCode'];
      }
    }
    if (this._nsMap.has(namespace)) {
...
class Constraint {
  ...
  _clonePropertiesTo(clone) {
    ...
    clone._lastModifiedBy = this._lastModifiedBy;
  }
...
class Value {
  ...
  toJSON() {
    const constraints = this._constraints.reduce((out, constraint, i) => {
      ...
      const valConstraints  = ['boolean', 'conceptCode'];
...
// const CIMCORE_VERSION = new VERSION(1.1);
